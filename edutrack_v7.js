...
const App = {
  ...
  /** يبني صف يوم كامل مع جميع حصصه */
  _createExamDayRow(data = {}) {
    const sessions = data.sessions || [
      { period: 'morning', subject: '', sciFrom: '08:00', sciTo: '10:00', litFrom: '08:00', litTo: '10:00' }
    ];
    const sessionsHtml = sessions.map((s, i) => this._sessionRowHtml(s, i)).join('');

    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td class="examday-info-cell" style="vertical-align:top;padding:10px 8px;min-width:170px">
        <input class="form-control sm" type="text"
          value="${this._esc(data.day || '')}"
          placeholder="الاثنين..." style="width:110px;margin-bottom:6px">
        <input class="form-control sm" type="date"
          value="${data.date || ''}" style="width:150px;margin-bottom:8px">
        <button class="btn btn-success btn-sm w-full"
          onclick="App._addSessionToDay(this)" style="font-size:11px">
          ➕ إضافة حصة
        </button>
      </td>
      <td style="padding:6px;vertical-align:top">
        <div class="sessions-container">${sessionsHtml}</div>
      </td>
      <td style="vertical-align:top;padding:10px 6px;text-align:center">
        <button class="btn btn-danger btn-sm btn-icon"
          onclick="this.closest('tr').remove()" title="حذف اليوم">✕</button>
      </td>
    `;
    return tr;
  },
  ...
  _sessionRowHtml(s = {}, idx = 0) {
    const periodOpts = ['morning','afternoon'].map(v =>
      `<option value="${v}" ${(s.period||'morning') === v ? 'selected' : ''}>${v === 'morning' ? '🕗 صباحاً' : '🕒 مساءً'}</option>`
    ).join('');
    ...
    return `
      <div class="session-block" data-idx="${idx}">
        <div class="session-header">
          <select class="form-control sm session-period" style="width:120px">${periodOpts}</select>
          <input class="form-control sm session-subject" type="text"
            value="${this._esc(s.subject || '')}"
            placeholder="اسم المادة..." style="flex:1;min-width:160px">
          <button class="btn btn-danger btn-sm btn-icon" style="flex-shrink:0"
            onclick="this.closest('.session-block').remove()" title="حذف الحصة">−</button>
        </div>
        <div class="session-times">
          <div class="time-track">
            <label class="track-label sci-label">
              <span class="track-dot" style="background:#1a6b3c">ع</span> علمي
            </label>
            <label class="track-na">
              <input type="checkbox" class="sci-na-cb" ${noSci ? 'checked' : ''}
                onchange="App._toggleNa(this,'sci')"> غير معني
            </label>
            <div class="time-inputs sci-times" ${noSci ? 'style="opacity:0.35;pointer-events:none"' : ''}>
              <input class="form-control sm sci-from" type="time"
                value="${s.sciFrom || '08:00'}" style="width:90px">
              <span class="track-arrow">→</span>
              <input class="form-control sm sci-to" type="time"
                value="${s.sciTo || '10:00'}" style="width:90px">
            </div>
          </div>
          <div class="time-track">
            <label class="track-label lit-label">
              <span class="track-dot" style="background:#c0392b">أ</span> أدبي
            </label>
            <label class="track-na">
              <input type="checkbox" class="lit-na-cb" ${noLit ? 'checked' : ''}
                onchange="App._toggleNa(this,'lit')"> غير معني
            </label>
            <div class="time-inputs lit-times" ${noLit ? 'style="opacity:0.35;pointer-events:none"' : ''}>
              <input class="form-control sm lit-from" type="time"
                value="${s.litFrom || '08:00'}" style="width:90px">
              <span class="track-arrow">→</span>
              <input class="form-control sm lit-to" type="time"
                value="${s.litTo || '10:00'}" style="width:90px">
            </div>
          </div>
        </div>
      </div>`;
  ...
  _addSessionToDay(btn) {
    ...
    div.innerHTML = this._sessionRowHtml({}, idx);
